@testset "multigrid hierarchy (Ngx_Mesh levels + parent maps)" begin
    ...
    P = parent_nodes(m)
    @test size(P) == (2, Netgen.GetNP(m))
    @test all(0 .<= P .<= Netgen.GetNP(m))
    # the newly added (fine) vertices must have two real parents
    nnew = count(j -> P[1, j] != 0 && P[2, j] != 0, 1:Netgen.GetNP(m))
    @test nnew > 0
    # original coarse vertices carry no parents
    @test count(j -> P[1, j] == 0 && P[2, j] == 0, 1:Netgen.GetNP(m)) >= np0
    PE = parent_elements(m)
    @test length(PE) == Netgen.GetNE(m)
end

@testset "copy_mesh is an independent deep copy" begin
    geom = load_step(STEP)
    m = generate_mesh(geom; maxh=40.0)
    np = Netgen.GetNP(m); ne = Netgen.GetNE(m)
    c = copy_mesh(m)
    @test Netgen.GetNP(c) == np
    @test Netgen.GetNE(c) == ne
    @test Netgen.GetDimension(c) == 3
    refine!(c)                            # refining the copy...
    @test Netgen.GetNE(c) > ne
    @test Netgen.GetNE(m) == ne           # ...leaves the original untouched
end
...
@testset "uniform_hierarchy builds nested GMG levels" begin
    ...
    @test nlevels(h) == 3
    np = [Netgen.GetNP(h[k]) for k in 1:3]
...
@testset "adaptive: grow hierarchy mid-simulation via refine_marked!" begin
    ...
    m2 = finest(h); ne2 = Netgen.GetNE(m2)
    refine_marked!(h, trues(ne2))   # mark all => uniform-like bisection
    @test nlevels(h) == 3
    @test Netgen.GetNE(finest(h)) > ne2
end

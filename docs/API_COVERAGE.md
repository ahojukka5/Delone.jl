...
| `Identifications` | `meshing/meshtype.hpp` | Read side (`GetNIdentifications`/`GetPeriodicVertices`) and axis-aligned-box write side (`OCC_IdentifyFacesBulk`/`OCC_RebuildGeometry` → `identify_periodic!`/`identify_periodic_box!`, including multi-fragment faces) now wrapped; the full point-index-level `Add`/`GetNr`/`SetName` API and arbitrary curved-face pairing remain unwrapped |

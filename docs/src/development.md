...
From the `Netgen.jl` package directory:
...
The `docs` environment depends on the parent package (`Netgen = {path = ".."}` in
...
4. Add Julian helper (if needed) in `Netgen.jl/src/`.
5. Add tests under `Netgen.jl/test/`.
